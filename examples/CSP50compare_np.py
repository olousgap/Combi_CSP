...
def economics_F(oTow:OutputContainer, csp_area_costs, csp_energy_price, csp_discount_rate,power_block_cost, Eoil, oil_price, lifetime=range(30)):
    ...
    dpb_tow = cspe.discounted_payback_period(csp_discount_rate, cash_flow_tow)
    npv_csp_tow = npf.npv(csp_discount_rate, [-capital_csp_tow] + [cspe.cashflow(oTow.Energy_MWh,csp_energy_price,Eoil,0.4,-oil_price,capital_csp_tow) for i in lifetime])
...
for A_helio in np.arange(75000,125001,10000): # 100MW np.arange(150000,250001,10000):
    ...
    tmp_res_Dic = economics_F(oTow=o_tmp, **econ_dic)
    area_list2.append(tmp_res_Dic['A_helio'] )
    cash_flow_list2.append(tmp_res_Dic['cash_flow_tow'])
    tow_scenaria2.append(tmp_res_Dic['tow_scenaria'])
    

#%% Assertions 
np.testing.assert_equal(area_list, area_list2) 
np.testing.assert_equal(cash_flow_list, cash_flow_list2) 
for k in range(len(tow_scenaria2)):
