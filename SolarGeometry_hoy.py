...
def ineichen():
    latitude, longitude, tz, altitude, name = 35, 24, 'Europe/Athens', 400, 'Ierapetra'
    ...
    return clearsky.ineichen(apparent_zenith, airmass, linke_turbidity, altitude, dni_extra)

def d1(hoy): # earth declination angle [in degrees]
    ...
    return -23.45 * cos(rad(360*(hoy+10*24)/365*24))
def d2(hoy):
...
def d(hoy): # [in radians] https://en.wikipedia.org/wiki/Sunrise_equation
...
def EoT(hoy): # equation of time [in minutes]
    gamma = 360*(hoy-1)/365
...
def tsol(hoy): # solar time [in decimal hours] introduce if function for east/west<<<<<<<<<
...
def W(hoy): # solar hour angle [in degrees]
...
def ele(hoy): # solar elevation angle or solar height [in radians]
    return asin(cos(rad(lat)) * cos(d(hoy)) * cos(rad(W(hoy)))
    + sin(rad(lat)) * sin(d(hoy)))
def z(hoy): # solar zenith angle [in radians] https://en.wikipedia.org/wiki/Solar_zenith_angle
    return acos(cos(rad(lat)) * cos(d(hoy)) * cos(rad(W(hoy))) 
    + sin(rad(lat)) * sin(d(hoy)))
def azim(hoy): # solar azimuth [in radians]
    return asin(cos(d(hoy)) * sin(rad(W(hoy))) / cos(ele(hoy)))
def thetai(hoy): # incidence angle [in radians]
    inclination=90
    azimuths=0
...
def I0(hoy): # Extra-terrestrial solar irradiance [W/m2]
    '''Appendix C in D.A. Katsaprakakis, Power Plant Synthesis, CRC Press, 2020.
    https://doi.org/10.1201/b22190.'''
    return 1373 * (1 + 0.033 * cos(rad(360*(hoy-3*24)/365*24)))

def AM(*args): # Air mass https://en.wikipedia.org/wiki/Air_mass_(solar_energy)
    for x in args:
        AM =  1 / cos(z(hoy))
    return AM
def AM2(hoy):
...
def Ib(hoy): # Direct irradiance [in W/m2]
...
def Ib2(alt): # Direct irradiance as function of height [in W/m2]
...
def Tr23km(alt): # Transmittance % Mid latitudes winter
...
def Tr5km(alt): # Transmittance % Mid latitudes winter
    '''H.C. Hottel, A simple model for estimating the transmittance of direct solar radiation through clear atmospheres, 
    Solar Energy. 18 (1976) 129–134.'''
...
def TrD23km(R): return 0.6739+10.46*R-1.7*R**2+0.2845*R**3
def TrD5km(R): return 1.293+27.48*R-3.394*R**2+0*R**3
def TrV23km(R): return 0.99326-0.1046*R+0.017*R**2-0.002845*R**3
def TrV5km(R): return 0.98707-0.2748*R+0.03394*R**2-0*R**3
